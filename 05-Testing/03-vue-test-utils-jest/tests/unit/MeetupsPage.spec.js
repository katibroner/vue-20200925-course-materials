...
describe('MeetupsPage', () => {
  ...
  it('should shallowMount mount snapshot', async () => {
    const wrapper = shallowMount(MeetupsPage);
    await flushPromises();
  ...
  it('should render list', async () => {
    ...
    await flushPromises();
    expect(fetchMeetups).toHaveBeenCalled();
    expect(wrapper.findAll('.meetups-list__item h5').wrappers.map(wrapper => wrapper.text())).toEqual([meetups[0].title, meetups[1].title]);
  });
});
